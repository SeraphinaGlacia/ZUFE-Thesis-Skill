...
它主要解决的是：**已经有 Word 初稿，不会 LaTeX，但想得到排版专业的文档** 的问题。本 Skill 会协助检查和准备环境，并将你的 Word 初稿转换为 LaTeX 工程与格式正确的 PDF，显著减少手动调整格式的时间，让你专注于文章内容本身。
...
## 三、使用这个 Skill，我需要准备什么？

你需要准备：

- 一份已经写好的 Word 初稿（需要使用 `.docx` 格式）
- 一份完整的 ZUFE-Thesis 模板项目
- 一个可以运行 Agent Skill 的工具环境（例如：Kimi Work、Codex 等）
...
> [!TIP]
> 转换前建议尽量保持 Word 文档中的章节标题、正文、图表标题和参考文献结构清晰，这能减少 Agent 二次确认和后续修改的次数。
