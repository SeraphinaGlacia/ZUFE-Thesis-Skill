...
scripts 只采集格式信号，例如样式名、字号、加粗、居中、编号、空行和段落位置。Agent 结合全文语义和格式信号做最终判断。
...
Agent 负责语义判断、正文归属、风险清单、用户确认和低置信度修正。
...
- 无规范标题样式 DOCX：Agent 能基于语义和格式信号识别章节，并列出低置信度问题。
