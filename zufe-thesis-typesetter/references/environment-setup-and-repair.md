...
默认不要先读本文件。环境判断先读 `environment-sop.md`；只有需要平台命令、安装细节、PATH 修复或 TeX 包补装时，才读取本文件对应小节。
...
```bash
python -c "import docx; print(docx.__version__)"
python "<skill-root>/scripts/check_env.py" --root . --stage minimal
```
...
kpsewhich biblatex.sty
kpsewhich gb7714-2015.bbx
python "<skill-root>/scripts/check_env.py" --root . --stage latex
```
