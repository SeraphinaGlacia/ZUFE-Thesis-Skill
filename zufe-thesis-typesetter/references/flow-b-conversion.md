...
脚本必须稳定处理高频正文转写问题。不要把这一节扩展成大而全的 LaTeX 语法清单；发现真实样本风险后，再把规则补入最小清单。

- 普通正文、标题、表格单元格、caption、metadata、摘要和关键词必须通过脚本正文转义。
- 特殊字符转义必须单次扫描完成，避免把脚本插入的 `\textbackslash{}`、`\textasciitilde{}` 等命令再次转义。
- 普通正文中的 ASCII 双引号 `"` 必须转换为 LaTeX 左右引号 ``...''。
- 中文智能引号 `“……”`、`‘……’` 默认保留。
- ASCII 单引号默认保留，避免误伤 `don't`、`student's` 等英文缩写或所有格。
- raw LaTeX、图片路径、引用命令和公式不得套用正文引号转换。
