...
Word 里的 `图2.1`、`图 2.1`、`表1.2` 等编号常常是普通手写文本，不一定是 Word 交叉引用。重排到 LaTeX 后章节号和图表顺序会重新计算，旧编号不得原样进入最终 PDF。
...
- 生成的 LaTeX `label` 必须唯一；每个 `\ref{...}`、`\autoref{...}`、`\pageref{...}` 或 `\eqref{...}` 都必须有对应 `\label{...}`。
